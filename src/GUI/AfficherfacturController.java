...
public class AfficherfacturController implements Initializable {
 ...
 ps.executeUpdate();
  ...
  private void btnPdf(ActionEvent evt) throws FileNotFoundException, DocumentException, IOException
    ...
    @FXML
    private void btnxlsxx(ActionEvent event) throws DocumentException, BadElementException, IOException {
        
        System.out.println("Boutton PDF Click");
        FacteurGrud sv = new FacteurGrud();
        ServiceExcel xlsx = new ServiceExcel();
        List<Facteur> listeFacteur = sv.afficherfacteurs();
        xlsx.genererxlsxFacteur("FACTUER", listeFacteur);
    }
    ...
    @FXML
      private void btnnext(ActionEvent event) {
        ...
        AjouterFicheAssurance mc = new AjouterFicheAssurance();
        mc.show();
    }
